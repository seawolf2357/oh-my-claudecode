...
def _is_fireworks_model(model: str) -> bool:
    return model.startswith("fireworks/") or model.startswith("accounts/fireworks/")


def _strip_fireworks_prefix(model: str) -> str:
    if model.startswith("fireworks/"):
        return model.removeprefix("fireworks/")
    return model
...
def make_llm_call(client, model, temperature, system_message, prompt):
    ...
    elif _is_fireworks_model(model):
        return client.chat.completions.create(
            model=_strip_fireworks_prefix(model),
            messages=[
                {"role": "system", "content": system_message},
                *prompt,
            ],
...
def get_response_from_vlm(
    ...
    if model in AVAILABLE_VLMS or _is_fireworks_model(model):
...
def create_client(model: str) -> tuple[Any, str]:
    ...
    if model in [
        ...
        client_model = model.removeprefix("fireworks/")
        print(f"Using Fireworks AI with model {client_model}.")
        return openai.OpenAI(

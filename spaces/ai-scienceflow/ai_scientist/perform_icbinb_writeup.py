...
def perform_writeup(
    ...
    try:
        ...
        if citations_text is None:
            ...
            if not citations_text:
                ...
                if citations_text is None:
                    ...
                    citations_text = ""
                gc.collect()  # Free memory after citation gathering
        ...
        # Initialize VLM client (used for descriptions and reflection reviews)
        vlm_client, vlm_model = None, None
        try:
            vlm_client, vlm_model = create_vlm_client(big_model)
        except Exception:
            print("WARNING: Could not create VLM client, VLM features will be skipped")
            print(traceback.format_exc())

        # Generate VLM-based descriptions
        try:
            if vlm_client is None:
                raise RuntimeError("VLM client not available")
            desc_map = {}
            ...
            del desc_map, plot_descriptions_list
            gc.collect()
        except Exception:
        ...
        # ReACT agent: generate section drafts with tool-augmented reasoning
        react_sections_text = ""
        try:
            from ai_scientist.react_writeup_agent import react_enhance_writeup
            print("[ReACT] Generating section drafts with tool-augmented reasoning...")
            react_sections = react_enhance_writeup(
                base_folder=base_folder,
                paper_title=idea_text[:200] if idea_text else "Research Paper",
                paper_abstract=idea_text[:1000] if idea_text else "",
                model=big_model,
                sections=["Introduction", "Related Work", "Methodology", "Experiments", "Conclusion"],
            )
            react_sections_text = "\n\n".join(
                f"=== ReACT Draft: {name} ===\n{content}"
                for name, content in react_sections.items()
            )
            print(f"[ReACT] Generated {len(react_sections)} section drafts")
            gc.collect()
        except Exception as e:
            print(f"[ReACT] ReACT agent failed (non-fatal): {e}")
            traceback.print_exc()
            react_sections_text = ""
        ...
        # Append ReACT drafts to summaries for the writeup LLM
        enhanced_summaries = combined_summaries_str
        if react_sections_text:
            enhanced_summaries += "\n\n--- ReACT Agent Section Drafts (use as reference) ---\n" + react_sections_text[:8000]
        ...
        for i in range(n_writeup_reflections):
            ...
            review_img_cap_ref = {}
            analysis_duplicate_figs = "No VLM client available for duplicate detection."
            if vlm_client is not None:
                try:
                    ...
                    print(f"VLM review failed: {vlm_err}")
                    review_img_cap_ref = {}
                ...
                try:
                    ...
                    print(f"VLM duplicate detection failed: {vlm_err}")
                    analysis_duplicate_figs = "Duplicate detection skipped."
...
"""
            ...
            if vlm_client is not None:
                try:
                    ...
                    print(f"VLM selection review failed: {vlm_err}")
                    review_img_selection = {}

...
def query(
    ...
    **model_kwargs,
    ...
    # Strip provider prefix from model name (removeprefix to avoid replacing inner occurrences)
    if filtered_kwargs.get("model", "").startswith("ollama/"):
       filtered_kwargs["model"] = filtered_kwargs["model"].removeprefix("ollama/")
    elif filtered_kwargs.get("model", "").startswith("fireworks/"):
       filtered_kwargs["model"] = filtered_kwargs["model"].removeprefix("fireworks/")
    ...
    if func_spec is None:
        ...
        # Strip <think> tags from models that include extended thinking

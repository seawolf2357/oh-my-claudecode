...
def extract_code_snippet(text: str) -> str:
    ...
    """
    # Primary: find code between ```python ... ``` or ``` ... ```
    pattern = r"```(?:python)?\s*\n(.*?)```"
    matches = re.findall(pattern, text, flags=re.DOTALL)
    if matches:
        # Use the longest match (most likely the full script)
        return max(matches, key=len).strip()
    # Fallback 1: original pattern without newline requirement
    pattern2 = r"```(?:python)?(.*?)```"
    matches2 = re.findall(pattern2, text, flags=re.DOTALL)
    if matches2:
        return max(matches2, key=len).strip()
    # Fallback 2: strip leading ```python and trailing ``` if present
    ...
    if lines and lines[0].strip().startswith('```'):
        ...
        if lines and lines[-1].strip() == '```':
            lines = lines[:-1]
        return '\n'.join(lines).strip()
    # Fallback 3: if text starts with valid Python, return as-is
    first_line = text.strip().split('\n')[0] if text.strip() else ''
    if first_line.startswith(('import ', 'from ', 'def ', 'class ', '#', 'os.')):
        return text.strip()
    return text.strip()
